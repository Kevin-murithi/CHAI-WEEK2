...
export default function FarmerAdvisory() {
  ...
  async function loadAdvisory() {
    try {
      ...
      const [res] = await Promise.all([api, delay])
      if (!res.ok) {
        const msg = await res.text().catch(() => '')
        throw new Error(`Failed to load advisory (${res.status}): ${msg || 'Unknown error'}`)
      }
      const data = await res.json()
  ...
  async function loadAnalytics(fieldId) {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = await res.text().catch(() => '')
        console.error('Analytics request failed:', res.status, msg)
      } else {
        const data = await res.json()
    ...
    <div className="space-y-4 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          <button onClick={loadAdvisory} disabled={loading} className="inline-flex items-center gap-2 rounded-lg bg-gradient-to-r from-blue-600 to-blue-700 text-white px-3 py-2 text-sm transition-all hover:from-blue-500 hover:to-blue-600 disabled:opacity-60 shadow-sm hover:shadow-blue-500/20">
